...
func getProduct(productID int) (*Product, error) {
	...
	defer cancel()
	row := database.DbConn.QueryRowContext(ctx, `SELECT * FROM products WHERE productId = ?`, productID)
	product := &Product{}
...
func GetTopTenProducts() ([]Product, error){
	ctx, cancel := context.WithTimeout(context.Background(), 15*time.Second)
	defer cancel()
	results, err := database.DbConn.QueryContext(ctx, `SELECT * FROM products ORDER BY quantityOnHand DESC LIMIT 10`)
	if err != nil {
...
func getProductList() ([]Product, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 15*time.Second)
	defer cancel()
	results, err := database.DbConn.QueryContext(ctx, `SELECT * FROM products`)
	if err != nil {
		return nil, err
	}
	defer results.Close()
...
func insertProduct(product Product) (int, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 15*time.Second)
	defer cancel()
	results, err := database.DbConn.ExecContext(ctx, `INSERT INTO products 
